// Create a function that takes channel_graph, and intro_node, max_length as input
// And outputs a Vec of all possible ChannnelRoutes

// ChannelRoute => Vec<ParsedEdge>
// ParsedEdge => Directional Edge info
// <channelid, from_node, to_node, capacity, from_node_policy>
...
// Goal is to find list of GoodRoutes aka Vec<Route>
...
struct Constraints {
    // Describes the total path length till now. Constraints by N
    path_length: u32,
    fee_base_msat: u64,
...
impl Default for Constraints {
    fn default() -> Self {
        Constraints {
            path_length: 1, // Default path length of 100
            fee_base_msat: 0, // Default fee base in millisatoshis
            htlc_minimum_msat: 0, // Default minimum HTLC value in millisatoshis
            cltv_expiry_delta: 0, // Default CLTV expiry delta
        }
    }
}
...
impl Add for Constraints {
    ...
    fn add(self, other: Constraints) -> Constraints {
        Constraints {
            ...
            fee_base_msat: self.fee_base_msat + other.fee_base_msat, // To be modified if able to somehow predict total amount forward
...
fn next_good_routes(current_route: &Route, channel_graph: &ChannelGraph, real_constraints: &Constraints) -> Option<Vec<Route>> {
    ...
    for (next_node, next_channel_id, next_channel_policy) in next_node_policies.iter() {
        // Condition 1 => The next_node is not already present in the route
...
pub fn get_final_routes(channel_graph: &ChannelGraph, blinded_path: &BlindedPath) -> Vec<Route> {
    ...
    // println!("Real constraints: {:?}", &real_constraints);
